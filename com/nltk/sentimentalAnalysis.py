...
import nltk, ast, pickle

try:
    fh = open("ptweets.txt", 'r', encoding='utf-8')
    string_tweets = fh.read()
    tuple_tweets = list(ast.literal_eval(string_tweets.replace("[", "").replace("]", "")))
#     print(tuple_tweets)
...
tweets = []
for (words, sentiment) in tuple_tweets:
...
def train(labeled_featuresets, estimator=nltk.probability.ELEProbDist):
    ...
    label_probdist = estimator(labeled_featuresets)
